            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm p-6 lg:p-8 min-h-[500px] flex flex-col">
                ...
                {loading ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {[1, 2, 3, 4, 5, 6].map(i => (
                            <div key={i} className="p-4 rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 animate-pulse flex flex-col items-center h-fit">
                                <div className="w-12 h-12 rounded-xl bg-slate-200 dark:bg-slate-700 mb-3" />
                                <div className="w-3/4 h-4 bg-slate-200 dark:bg-slate-700 rounded mb-2" />
                                <div className="w-1/2 h-3 bg-slate-200 dark:bg-slate-700 rounded mb-3" />
                                <div className="w-full h-8 bg-slate-200 dark:bg-slate-700/30 rounded-xl" />
                            </div>
                        ))}
                    </div>
                    ...
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 flex-1">
                            {filteredColleagues.map(colleague => (
                                <div 
                                    key={colleague.id} 
                                    className="group p-4 rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col items-center text-center relative overflow-hidden h-fit"
                                >
                                    {/* Decorative top bar */}
                                    <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 to-blue-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                                    
                                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center text-base font-black mb-3 border border-blue-100 dark:border-blue-800">
                                        {colleague.name.charAt(0).toUpperCase()}
                                    </div>
                                    
                                    <h4 className="font-bold text-sm text-slate-900 dark:text-white truncate w-full">{colleague.name}</h4>
                                    <p className="text-[11px] text-slate-500 truncate w-full mt-0.5 mb-3">{colleague.email}</p>
                                    
                                    <a 
                                        href={`mailto:${colleague.email}?subject=${encodeURIComponent("LAMAS Academic Update")}`}
                                        className="mt-auto w-full py-2 rounded-xl bg-slate-50 dark:bg-slate-700/50 hover:bg-blue-50 dark:hover:bg-blue-900/30 text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 font-bold text-xs transition-colors flex items-center justify-center gap-1.5 border border-transparent hover:border-blue-200 dark:hover:border-blue-800"
                                    >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto flex flex-col border border-slate-200 dark:border-slate-700 animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-4 sm:p-6">
                            <div className="mb-6">
                                <p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">To</p>
                                <div className="flex items-center gap-2">
                                    <div className="w-6 h-6 rounded-md bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center text-xs font-black">
                                        {targetId === "ALL" ? <Megaphone className="w-3.5 h-3.5" /> : targetName.charAt(0).toUpperCase()}
                                    </div>
                                    <span className="font-bold text-slate-900 dark:text-white text-sm truncate max-w-full">{targetName}</span>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleSend}>
                                <div className="mb-6">
                                    <label className="block text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Message Content</label>
                                    <textarea
                                        value={message}
                                        onChange={e => setMessage(e.target.value)}
                                        rows={5}
                                        placeholder="Type your message here..."
                                        className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700/50 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none resize-none text-sm"
                                        autoFocus
                                    />
                                </div>
                                <div className="flex flex-col-reverse sm:flex-row sm:items-center justify-end gap-3 pt-2">
                                    <button
                                        type="button"
                                        onClick={() => setIsModalOpen(false)}
                                        className="w-full sm:w-auto px-5 py-3 sm:py-2.5 rounded-xl font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 text-sm transition-colors text-center"
                                    >
                                        Cancel
                                    </button>
                                    <button
                                        ...
                                        className="w-full sm:w-auto px-6 py-3 sm:py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm transition-colors shadow-md shadow-blue-500/20 disabled:opacity-50 flex items-center justify-center gap-2"
                                    >
