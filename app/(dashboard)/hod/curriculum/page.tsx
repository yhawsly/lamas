...
export default function HODCurriculumPage() {
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 p-6 md:p-10">
            {/* Header */}
            <div>
                ...
                <p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>
                    Explore program curriculum matrices, identify general courses, and staff sections.
                </p>
            </div>
                            ...
                            <div key={level} className="rounded-2xl border overflow-hidden" style={{ backgroundColor: "var(--bg-surface)", borderColor: "var(--bg-border)" }}>
                                ...
                                <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: "var(--bg-border)", backgroundColor: "var(--bg-hover)" }}>
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-500 font-black text-sm animate-pulse">
                                            {level === 500 ? "PG" : level}
                                        </div>
                                        <div>
                                                            ...
                                                            return (
                                                                <div
                                                                    ...
                                                                    className={`group flex flex-col p-4 rounded-xl border transition cursor-pointer hover:-translate-y-0.5 hover:shadow-md ${
                                                                        isDept 
                                                                            ? "border-blue-500/30 hover:border-blue-500 bg-blue-500/[0.01]" 
                                                                ...
                                                                >
                                                                        ...
                                                                        {entry.course.sections && entry.course.sections.length > 0 ? (
                                                                            entry.course.sections.map(s => (
                                                                                <div key={s.id} className="flex items-center justify-between text-[10px] font-medium text-slate-400">
                                                                                    ...
                                                                                    {s.lecturer ? (
                                                                                        <span className="font-bold text-slate-300 truncate max-w-[100px]">👤 {s.lecturer.name}</span>
                                                                                    ) : (
                                                                                        <span className="font-bold text-amber-500">⚠️ Unassigned</span>
                                                                                    )}
                                                                                </div>
                                                                            ))
                                                                        ) : (
                                                                    ...
                                                                    {isDept && hasUnassigned && (
                                                                        <div className="mt-2.5 flex items-center gap-1.5 text-[9px] font-bold text-amber-500 uppercase tracking-wide">
                ...
                <div className="fixed inset-0 z-[500] flex items-center justify-end bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="w-full max-w-md h-full shadow-2xl p-6 overflow-y-auto flex flex-col justify-between animate-in slide-in-from-right duration-300" style={{ backgroundColor: "var(--bg-base)" }}>
                        <div className="space-y-6">
                            ...
                            {activeCourse.departmentId !== userDeptId && (
                                <div className="p-4 rounded-xl text-xs leading-relaxed border bg-slate-500/5 text-slate-400" style={{ borderColor: "var(--bg-border)" }}>
                                    📢 <strong className="text-slate-300">Read-Only View:</strong> This course is offered and staffed by the <strong className="text-slate-300">{activeCourse.department?.name || "another"} department</strong>. You can monitor the staffing assignments below, but changes must be made by the offering department HOD.
                                </div>
                            )}
                                ...
                                {showAddSection && activeCourse.departmentId === userDeptId && (
                                    <div className="p-4 rounded-xl border space-y-3" style={{ backgroundColor: "var(--bg-hover)", borderColor: "var(--bg-border)" }}>
                                        ...
                                        <select
                                            ...
                                            className="w-full bg-white dark:bg-slate-800 border rounded-lg px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-350 outline-none focus:ring-2 focus:ring-blue-500/30 transition"
                                            style={{ borderColor: "var(--bg-border)" }}
                                        >
                        ...
                        <div className="mt-8 pt-4 border-t" style={{ borderColor: "var(--bg-border)" }}>
                            <button
                                ...
                                className="w-full py-2.5 bg-slate-500/10 hover:bg-slate-500/20 text-slate-400 rounded-xl text-xs font-bold transition"
                            >
                                Close Details
                            </button>
                        </div>
