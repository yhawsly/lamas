        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="rounded-2xl p-6" style={{ backgroundColor: "var(--bg-surface)", border: "1px solid var(--bg-border)" }}>
                    <h3 className="font-semibold mb-5" style={{ color: "var(--text-primary)" }}>Generate Report</h3>
                    <div className="space-y-4">
                        {Object.entries(descriptions).map(([key, desc]) => (
                            <button key={key} onClick={() => setType(key)}
                                className="w-full text-left p-4 rounded-xl border transition"
                                style={{
                                    backgroundColor: type === key ? "rgba(79, 70, 229, 0.12)" : "var(--bg-hover)",
                                    borderColor: type === key ? "var(--primary)" : "var(--bg-border)",
                ...
                <div className="rounded-2xl p-6" style={{ backgroundColor: "var(--bg-surface)", border: "1px solid var(--bg-border)" }}>
                    ...
                    {reports.length === 0 ? (
                        <div className="text-center py-12" style={{ color: "var(--text-muted)" }}>
                            <div className="text-4xl mb-3">Report</div>
                        ...
                        <div className="space-y-3">
                            {reports.map((r, i) => (
                                <div key={i} className="p-4 rounded-xl" style={{ backgroundColor: "var(--bg-hover)", border: "1px solid var(--bg-border)" }}>
                                    ...
                                    <div className="text-xs mt-2" style={{ color: "var(--accent-amber)" }}>{r.note}</div>
                                </div>
                            ))}
                        </div>
                    )}
