...
export default function ConductModerationPage() {
    ...
    if (loading || !data) return <Loader message="Synchronizing Moderation Artifact..." />;
    ...
    const renderExamQRow = (field: keyof FormCReviewData["examQuestions"], sn: number, typeLabel: string) => (
        <tr className="border-t hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors" style={{ borderColor: "var(--bg-border)" }}>
            ...
            <td className="py-3 px-4 text-center border-l" style={{ borderColor: "var(--bg-border)" }}>
                <input type="checkbox" disabled={isCompleted} checked={reviewData.examQuestions[field].yes} onChange={e => setReviewData(p => ({ ...p, examQuestions: { ...p.examQuestions, [field]: { ...p.examQuestions[field], yes: e.target.checked } } }))} className="w-4 h-4 text-primary" />
            </td>
            <td className="py-3 px-4 text-center border-l" style={{ borderColor: "var(--bg-border)" }}>
                <input type="checkbox" disabled={isCompleted} checked={reviewData.examQuestions[field].no} onChange={e => setReviewData(p => ({ ...p, examQuestions: { ...p.examQuestions, [field]: { ...p.examQuestions[field], no: e.target.checked } } }))} className="w-4 h-4 text-primary" />
            </td>
            <td className="py-3 px-4 border-l" style={{ borderColor: "var(--bg-border)" }}>
                <input type="number" disabled={isCompleted} value={reviewData.examQuestions[field].numQuestions} onChange={e => setReviewData(p => ({ ...p, examQuestions: { ...p.examQuestions, [field]: { ...p.examQuestions[field], numQuestions: e.target.value } } }))} className="w-full bg-transparent outline-none text-center" />
            </td>
            <td className="py-3 px-4 border-l" style={{ borderColor: "var(--bg-border)" }}>
                <input type="number" disabled={isCompleted} value={reviewData.examQuestions[field].numToAnswer} onChange={e => setReviewData(p => ({ ...p, examQuestions: { ...p.examQuestions, [field]: { ...p.examQuestions[field], numToAnswer: e.target.value } } }))} className="w-full bg-transparent outline-none text-center" />
            </td>
        </tr>
    );
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            {/* Header */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end pb-6 border-b" style={{ borderColor: "var(--bg-border)" }}>
                <div>
                    <h1 className="text-3xl font-bold tracking-tight mb-2" style={{ color: "var(--text-primary)" }}>
                        Examination Questions & Marking Schemes Internal Moderation Report
                    </h1>
                ...
                <div className={`mt-4 md:mt-0 px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest uppercase border ${data.status === "PENDING"
                    ...
                    <div className="flex gap-6">
                        {[
                            ...
                            <label key={m.key} className="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" disabled={isCompleted} checked={reviewData.natureOfExam[m.key as keyof typeof reviewData.natureOfExam]} onChange={e => setReviewData(p => ({ ...p, natureOfExam: { ...p.natureOfExam, [m.key]: e.target.checked } }))} className="w-5 h-5 rounded border-gray-300 text-primary" />
                ...
                <div className="rounded-2xl p-6 shadow-sm border" style={{ backgroundColor: "var(--bg-surface)", borderColor: "var(--bg-border)" }}>
                    ...
                    <div className="flex flex-wrap gap-4">
                        {[
                            ...
                            <label key={m.key} className="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" disabled={isCompleted} checked={reviewData.materialsReviewed[m.key as keyof typeof reviewData.materialsReviewed]} onChange={e => setReviewData(p => ({ ...p, materialsReviewed: { ...p.materialsReviewed, [m.key]: e.target.checked } }))} className="w-5 h-5 rounded border-gray-300 text-primary" />
                                <span style={{ color: "var(--text-secondary)" }}>{m.label}</span>
                            </label>
                        ))}
                    </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <div className="rounded-2xl shadow-sm border p-6" style={{ borderColor: "var(--bg-border)", backgroundColor: "var(--bg-surface)" }}>
                    ...
                    <ol className="list-[lower-roman] list-inside space-y-2 text-sm" style={{ color: "var(--text-primary)" }}>
                        {[0, 1, 2, 3, 4].map(idx => (
                            <li key={idx}>
                                <input type="text" disabled={isCompleted} value={reviewData.changesExamQuestions[idx]} onChange={e => { const arr = [...reviewData.changesExamQuestions]; arr[idx] = e.target.value; setReviewData(p => ({ ...p, changesExamQuestions: arr })) }} className="bg-transparent border-b outline-none w-[90%] px-2 py-1 ml-2" style={{ borderColor: "var(--bg-border)" }} />
                            </li>
                        ))}
                    </ol>
                </div>
                <div className="rounded-2xl shadow-sm border p-6" style={{ borderColor: "var(--bg-border)", backgroundColor: "var(--bg-surface)" }}>
                    ...
                    <ol className="list-[lower-roman] list-inside space-y-2 text-sm" style={{ color: "var(--text-primary)" }}>
                        {[0, 1, 2, 3, 4].map(idx => (
                            <li key={idx}>
                                <input type="text" disabled={isCompleted} value={reviewData.changesMarkingScheme[idx]} onChange={e => { const arr = [...reviewData.changesMarkingScheme]; arr[idx] = e.target.value; setReviewData(p => ({ ...p, changesMarkingScheme: arr })) }} className="bg-transparent border-b outline-none w-[90%] px-2 py-1 ml-2" style={{ borderColor: "var(--bg-border)" }} />
                            </li>
                        ))}
                    </ol>
                </div>
            ...
            <div className="rounded-2xl shadow-sm border p-6" style={{ borderColor: "var(--bg-border)", backgroundColor: "var(--bg-surface)" }}>
                ...
                <textarea disabled={isCompleted} value={reviewData.generalComments} onChange={e => setReviewData(p => ({ ...p, generalComments: e.target.value }))} className="w-full h-32 bg-transparent border rounded-lg p-4 outline-none resize-none" style={{ borderColor: "var(--bg-border)", color: "var(--text-primary)" }} />
            </div>
                    ...
                    <div className="flex flex-wrap gap-4">
                        {["Excellent", "Very Good", "Good", "Fair", "Poor"].map(r => (
                            <label key={r} className="flex items-center gap-2 cursor-pointer border px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50" style={{ borderColor: reviewData.overallRatingExam === r ? "var(--primary)" : "var(--bg-border)" }}>
                                <input type="radio" name="rateExam" disabled={isCompleted} checked={reviewData.overallRatingExam === r} onChange={() => setReviewData(p => ({ ...p, overallRatingExam: r as any }))} className="w-4 h-4 text-primary" />
                    ...
                    <div className="flex flex-wrap gap-4">
                        {["Excellent", "Very Good", "Good", "Fair", "Poor"].map(r => (
                            <label key={r} className="flex items-center gap-2 cursor-pointer border px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50" style={{ borderColor: reviewData.overallRatingMarking === r ? "var(--primary)" : "var(--bg-border)" }}>
                                <input type="radio" name="rateMarking" disabled={isCompleted} checked={reviewData.overallRatingMarking === r} onChange={() => setReviewData(p => ({ ...p, overallRatingMarking: r as any }))} className="w-4 h-4 text-primary" />
                                <span className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>{r}</span>
                            </label>
                        ))}
                    </div>
            ...
            {/* Error */}
            {data.status === "PENDING" && (
                <p className="text-center text-sm font-bold opacity-50 uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>This report is pending completion by the assigned moderator.</p>
            )}
