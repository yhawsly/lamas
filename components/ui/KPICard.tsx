            ...
            <div className="relative z-10">
                <div className={`flex items-center justify-between ${isSm ? "mb-1.5 sm:mb-2" : "mb-2 sm:mb-3"}`}>
                    <div className="flex items-center gap-2.5 sm:gap-4 min-w-0">
                        <div className={`relative flex items-center justify-center shrink-0 transition-transform duration-300 group-hover:scale-110 ${isSm ? "p-2 sm:p-2.5 rounded-lg sm:rounded-xl" : "p-2 sm:p-3 rounded-xl sm:rounded-2xl"}`}>
                            {/* Background with opacity */}
                            <div className={`absolute inset-0 opacity-15 ${isSm ? "rounded-lg sm:rounded-xl" : "rounded-xl sm:rounded-2xl"}`} style={{ backgroundColor: color }} />
                            {/* Icon */}
                            <div className="relative z-10 [&>svg]:w-4 [&>svg]:h-4 sm:[&>svg]:w-6 sm:[&>svg]:h-6" style={{ color }}>{icon}</div>
                        </div>
